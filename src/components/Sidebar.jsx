...
export default function Sidebar() {
  const { currentUser, logout, darkMode } = useGlobal();
  const location = useLocation();
  const [active, setActive] = useState(location.pathname.split("/")[1] || "dashboard");
  ...
  return (
    <aside className={`bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 w-64 min-h-screen flex flex-col`}>
      <div className="flex items-center gap-2 mb-10 p-6">
        ...
        <div>
          <h1 className="font-bold text-lg text-gray-900 dark:text-gray-100 mt-2">iHUZA</h1>
          <p className="text-xs text-gray-500 dark:text-gray-400 tracking-wide">INVENTORY</p>
        </div>
      </div>
